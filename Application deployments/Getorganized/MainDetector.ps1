...
[string]$ExpectedVersion            = "6.33.0712"
...
Write-ToLog "Starting detection script for GetOrganized" -IsHeader
...
if ($installedProduct) {
    ...
    if ($detectedVersion) { $detectedVersion = $detectedVersion.Trim() }
    Write-ToLog "Detected version from registry: $detectedVersion (Product: $($installedProduct.DisplayName))" "Cyan"
}

if ($detectedVersion) {
    Write-ToLog "Detected GetOrganized version: $detectedVersion" "Cyan"
    ...
    Write-ToLog "GetOrganized not detected (no version found in registry)." "Red"
    Write-ToLog "Ending detection script" -IsHeader
    ApplicationNotDetected
}
#endregion